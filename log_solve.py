...
def dot(x, y):
    if y.ndim == 1:
        return torch_semiring_einsum.log_einsum_forward(vv_eq, x, y, block_size=10)
    elif y.ndim == 2:
        return torch_semiring_einsum.log_einsum_forward(vm_eq, x, y, block_size=10)

def matmul(x, y):
    return torch_semiring_einsum.log_einsum_forward(mm_eq, x, y, block_size=10)
    
outer_eq = torch_semiring_einsum.compile_equation('i,j->ij')
def outer(x, y):
    return torch_semiring_einsum.log_einsum_forward(outer_eq, x, y, block_size=10)
